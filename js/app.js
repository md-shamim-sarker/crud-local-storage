...
const display = () => {
    ...
    personsArray.forEach(element => {
        ...
        tr.innerHTML = `
            ...
            <td>${element.name}</td>
            <td>${element.age}</td>
            <td><button class="btn btn-success" onclick="updateItem(this.value)" value="${element.name}">Update</button></td>
            <td><button class="btn btn-danger" onclick="removeItem(this.value)" value="${element.name}">Delete</button></td>
        `;
